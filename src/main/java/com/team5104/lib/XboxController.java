...
import com.team5104.lib.XboxController.Button.ButtonType;
...
public class XboxController {
  ...
  public static void update() {
    for (XboxController controller : controllers) {
      for (Button button : controller.buttons) button.update();
      if (controller.activeRumble != null) controller.activeRumble.update();
  ...
  public static class Button {
    ...
    void update() {
      ...
      if (doubleClickTime != -1
          && System.currentTimeMillis() > doubleClickTime + doubleClickKilloff) {
        // Killoff
        doubleClickIndex = 1;
        doubleClickTime = -1;
      }
    ...
    /**
     * Returns true on event trigger - Normal: button pressed event - Hold: if button is down - Hold
     * Time: held event - Double Click: double click event
     */
    public boolean get() {
      if (buttonType == ButtonType.NORMAL) return pressed;
      if (buttonType == ButtonType.HOLD) return isDown();
      if (buttonType == ButtonType.HOLD_TIME) {
        boolean temp =
            ((value ? ((double) (System.currentTimeMillis() - heldEventTime)) : 0)
                    > heldEventLength)
                && (!heldEventReturned);
        if (temp) heldEventReturned = true;
        return temp;
      }
      if (buttonType == ButtonType.DOUBLE_CLICK) return doubleClickIndex == 2;
      return false;
    }

    /**
     * Returns an alternative trigger depending on button type. - Normal: returns button released
     * action. - Hold: returns nothing - Hold Time: returns nothing. - Double Click: returns if the
     * double click was killed off with only 1 button pressed (event).
     */
    public boolean getAlt() {
      if (buttonType == ButtonType.NORMAL) return released;
      if (buttonType == ButtonType.DOUBLE_CLICK) return doubleClickIndex == 1;
      return false;
    }
  ...
  private static class DoubleButton extends Button {
    ...
    /**
     * Ors the result of get() on both buttons Returns true on event trigger (button pressed, held
     * event, or double click)
     */
    public boolean get() {
    ...
    /**
     * Ors the result of getAlt() on both buttons Returns an alternative trigger depending on button
     * type. - Normal: returns button released action. - Hold: returns nothing. - Double Click:
     * returns if the double click was killed off with only 1 button pressed (event).
     */
    public boolean getAlt() {
  ...
  public static class Axis {
    ...
    public double get() {
      ...
      if (curve != null) val = curve.getPoint(val);
      return val;
    }

    private double getRawAxisValue() {
      if (isConnected(port)) return DriverStation.getInstance().getStickAxis(port, slot);
      return 0;
    }

    // Settings
    public void changeCurveX1(double x1) {
      if (curve != null) curve.x1 = x1;
    }
  ...
  public Rumble rumble(double strength, boolean hard, int timeoutMs) {
    return rumble(strength, hard, timeoutMs, 0);
  }
  ...
  public static class Rumble {
    ...
    void update() {
      ...
      for (int i = 0; i < dipCount; i++) {
        if (x > (0.4 / dipCount) + ((double) i / dipCount)
            && x < (0.6 / dipCount) + ((double) i / dipCount)) on = false;
      }
      setRumble(on);
    ...
    private void setRumble(boolean on) {
      if (isConnected(port)) {
        if (on) HAL.setJoystickOutputs((byte) port, 0, hard ? strength : 0, hard ? 0 : strength);
  ...
  public static class XboxControllerGroup {
    ...
    // Buttons
    public Button button(int slot) {
      return new DoubleButton(controller1.button(slot), controller2.button(slot));
    }

    public Button holdButton(int slot) {
      return new DoubleButton(controller1.holdButton(slot), controller2.holdButton(slot));
    }

    public Button holdTimeButton(
        int slot, int holdTime, XboxController controller1, XboxController controller2) {
      return new DoubleButton(
          controller1.holdTimeButton(slot, holdTime), controller2.holdTimeButton(slot, holdTime));
    }
    ...
    // Axis'
    public Axis axis(int slot) {
      return new DoubleAxis(controller1.axis(slot), controller2.axis(slot));
    }

    public Axis axis(int slot, Deadband deadband) {
      return new DoubleAxis(controller1.axis(slot, deadband), controller2.axis(slot, deadband));
    }

    public Axis axis(int slot, Deadband deadband, BezierCurve curve) {
      return new DoubleAxis(
          controller1.axis(slot, deadband, curve), controller2.axis(slot, deadband, curve));
    }
  ...
  // Bezier Curves
  /**
   * Processes bezier curves between two points. Desmos Link:
   * https://www.desmos.com/calculator/da8zwxpgzo
   */
  ...
  // Deadband
  /**
   * A deadband cuts out areas of an input. For example in a clipping deadband with a radius of .05,
   * .05 would go to 0 and .06 would not change. This class has two deadbands. - Clipping: in which
   * areas will be directly cut out (r=.05: .05->0, .06->.06) - Slope Adjustment: in which the slope
   * is adjusted (r=.05: .05->0, 0.06->0.01) Desmos Link:
   * https://www.desmos.com/calculator/xhbilptzt9
   */
  public static class Deadband {
    // Deadband Types

    /**
     * Clipping: in which areas will be directly cut out (r=.05: .05->0, .06->.06) Slope Adjustment:
     * in which the slope is adjusted (r=.05: .05->0, 0.06->0.01)
     */
    ...
    public static double get(double x, double radius, DeadbandType type) {
      ...
      if (type == DeadbandType.CLIPPING) return getClipping(Math.abs(x), radius) * (x > 0 ? 1 : -1);
    ...
    public Deadband() {
      this(0, DeadbandType.NONE);
    }

    public Deadband(double radius) {
      this(radius, DeadbandType.SLOPE_ADJ);
    }
