...
public class RobotController extends RobotBase {
  ...
  public void startCompetition() {
    // Logging
    console.init();
    if (!RobotState.isSimulation()) console.logFile.start();
    ...
    // Set Child Class
    if (RobotState.isSimulation()) robot = new RobotSim();
    ...
    while (true) {
      ...
      if (NotifierJNI.waitForNotifierAlarm(notifier) == 0) break;
    ...
    try {
      if (RobotState.getLastMode() != RobotState.getMode()) {
        if (RobotState.getMode() == RobotMode.DISABLED) {
          if (!RobotState.isSimulation()) console.logFile.end();
          robot.mainStop();
  ...
  // Child Class
  /**
   * The Main Robot Interface. Called by this, Breaker Robot Controller <br>
   * Override these methods to run code <br>
   * Functions Call Order: <br>
   * - All Enable/Disable Functions are called before the corresponding loop function <br>
   * - Main Functions are called last (teleop, test, auto are before)
   */
  public abstract static class BreakerRobot {
