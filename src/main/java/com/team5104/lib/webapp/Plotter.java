...
/** Handles the plotting of points on the WebApp. For WebApp version 2.6 */
public class Plotter {
  ...
  public static void plotAll(List<Position> positions, Color color) {
    for (int i = 0; i < positions.size(); i++) {
      plot(positions.get(i).getXFeet(), positions.get(i).getYFeet(), color);
    }
  }
  ...
  public static void onInput(String data) {
    if (inputListener != null) inputListener.accept(data);
  }
