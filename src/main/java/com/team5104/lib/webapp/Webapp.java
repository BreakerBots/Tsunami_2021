...
/**
 * Hosts the BreakerBoard (WebApp) through the RoboRIO. See Tuner.java for the tuner functionality.
 *
 * @version 3.2
 */
public class Webapp {
  ...
  public static String getBaseUrl() {
    if (RobotState.isSimulation()) return System.getProperty("user.dir") + "\\src\\webapp\\";
  ...
  public static String getJSONValue(String json, String key) {
    ...
    if (keyI == -1) return null;
    keyI += key.length();

    int endI = json.indexOf(",", keyI);
    if (endI == -1) endI = json.indexOf("}", keyI);
