...
/**
 * Handles all tuner inputs and outputs for the WebApp. Either add the annotations @tunerOutput
 * (graphing) and @tunerInput (inputs) to methods and fields and plug in the class with
 * Tuner.init(CLASS) or use the new methods "getTunerInput()" and "setTunerOutput()" For WebApp
 * version 2.6
 */
public class Tuner {
  private static final ArrayList<TunerRunnable> runnables = new ArrayList<TunerRunnable>();
  ...
  public static void use(Class<?>... targets) {
    for (Class<?> target : targets) {
      for (Field field : target.getDeclaredFields()) {
        ...
        if (field.getAnnotation(tunerInput.class) != null
            || field.getAnnotation(tunerOutput.class) != null)
          runnables.add(new TunerRunnable(field));
      }
      for (Method method : target.getDeclaredMethods()) {
        method.setAccessible(true);
        if (method.getAnnotation(tunerInput.class) != null
            || method.getAnnotation(tunerOutput.class) != null)
  ...
  protected static class TunerRunnable {
    ...
    public TunerRunnable(Field target) {
      ...
      if (name.equals("")) name = fieldTarget.getName();
    }
    ...
    public TunerRunnable(Method target) {
      ...
      if (name.equals("")) name = methodTarget.getName();
    }
