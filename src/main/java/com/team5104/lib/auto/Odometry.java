...
public class Odometry {
  ...
  public static Position getPositionFeet() {
    if (odometry == null) return new Position(0, 0, 0);
    return new Position(
  ...
  public static DifferentialDriveWheelSpeeds getWheelSpeeds() {
    return new DifferentialDriveWheelSpeeds(
        Drive.getDriveStateMeters()[2], Drive.getDriveStateMeters()[3]);
  }
  ...
  public static void reset(Position position) {
    if (odometry != null) {
      ...
      odometry.resetPosition(
          position.toPose2dMeters(), Rotation2d.fromDegrees(position.getDegrees()));
    }
  }
}
