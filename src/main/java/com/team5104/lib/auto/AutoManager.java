...
public class AutoManager {
  ...
  public static void disabled() {
    ...
    if (Characterizer.isRunning()) Characterizer.disabled();
  }
  ...
  public static void update() {
    // update the path
    if (targetPath != null) targetPath.update();

    // update characterization
    if (Characterizer.isRunning()) Characterizer.update();

    // stop compressor
    if (!RobotState.isSimulation()) CompressorController.stop();
  ...
  public static void runTrajectoryTester() {
    ...
    Plotter.setInputListener(
        (data) -> {
          boolean reversed = Webapp.getJSONValue(data, "reversed") == "\"true\"";
          String[] pointStrings =
              data.substring(data.indexOf("points\":") + 9, data.indexOf("}") - 1)
                  .replaceAll("\"", "")
                  .split("]");
