...
public class SubsystemManager {
  ...
  public static void useSubsystems(Subsystem... availableSubsystems) {
    ...
    for (Subsystem subsystem : targetSubsystems) {
      try {
        ...
        if (RobotState.isSimulation()) subsystem.initSim();
        else subsystem.init();
    ...
    Looper.attach(
        () -> {
          for (Subsystem subsystem : targetSubsystems) {
            try {
              if (!subsystem.emergencyStopped) subsystem.fastUpdate();
            } catch (Exception e) {
              Looper.logCrash(new Crash(e));
            }
          }
  ...
  public static void debug(Class<? extends Subsystem>... subsystemsToDebug) {
    for (Class<? extends Subsystem> subsystemToDebug : subsystemsToDebug) {
      for (Subsystem subsystem : targetSubsystems) {
        try {
          if (subsystem.getClass() == subsystemToDebug) subsystem.startDebugging();
        } catch (Exception e) {
          Looper.logCrash(new Crash(e));
        }
      }
  ...
  public static Subsystem getSubsystem(Class<? extends Subsystem> targetSubsystem) {
    for (Subsystem subsystem : targetSubsystems) {
      try {
        if (subsystem.getClass() == targetSubsystem) return subsystem;
      } catch (Exception e) {
        Looper.logCrash(new Crash(e));
      }
    }
  ...
  public static void update() {
    for (Subsystem subsystem : targetSubsystems) {
      try {
        if (subsystem.emergencyStopped) subsystem.disabled();
        else {
          subsystem.update();
          if (RobotState.isSimulation()) subsystem.updateSim();
          if (subsystem.debugging) subsystem.debug();
        }
      } catch (Exception e) {
        Looper.logCrash(new Crash(e));
      }
