...
public abstract class Subsystem {
  ...
  /**
   * Labels this subsystem as emergency stopped. Stops calling the update() and fastUpdate() methods
   * and instead spam calls disabled().
   */
  ...
  // Sim
  /**
   * Called periodically from the robot loop (along with update() and fastUpdate()) while in
   * simulation
   *
   * @default nothing
   */
  ...
  public long getTimeInCalibration() {
    if (isCalibrating()) return System.currentTimeMillis() - calibrationStart;
    else return 0;
  }

  // Characterization
  private DoubleSupplier getLeftEncoderPos,
      getLeftEncoderVel,
      getRightEncoderPos,
      getRightEncoderVel,
      getGyroAngleRadians;
  ...
  protected void configCharacterization(Encoder encoder, DoubleConsumer outputVoltage) {
    configCharacterization(
        () -> encoder.getComponentRevs(), () -> encoder.getComponentRPS(), outputVoltage);
  }

  protected void configCharacterization(
      DoubleSupplier encoderPos, DoubleSupplier encoderVel, DoubleConsumer outputVoltage) {
    configCharacterization(
        encoderPos,
        encoderVel,
        encoderPos,
        ...
        () -> 0,
        (Double l, Double r) -> {
          outputVoltage.accept(l);
        });
  }

  protected void configCharacterization(
      DoubleSupplier leftEncoderPos,
      DoubleSupplier leftEncoderVel,
      DoubleSupplier rightEncoderPos,
      DoubleSupplier rightEncoderVel,
  ...
  public double[] getCharacterization(double percentOutput, boolean shouldRotate) {
    if (!canCharacterize)
      console.error(
          "You have not set up characterization for this setup. Call the configCharacterization() method in the init() method add an `if (isCharacterizing())` statement to your update() method so it doesnt move motors.");
