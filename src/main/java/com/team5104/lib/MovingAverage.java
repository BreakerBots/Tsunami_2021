...
/**
 * Simply smoothes out a input by using an average between the last n inputs. Useful for sensors
 * that give variable outputs or any variable that spikes.
 */
public class MovingAverage {
  ...
  public MovingAverage(int n, double init) {
    this.init = init;
    values = new double[n];
    for (int i = 0; i < n; i++) values[i] = init;
  }
  ...
  // Reset
  public void reset() {
    for (int i = 0; i < values.length; i++) values[i] = init;
  }
