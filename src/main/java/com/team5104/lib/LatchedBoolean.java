...
/**
 * A latched boolean is a boolean that only returns true when the input changes values. For example
 * when a boolean changes from false->true or true->false. This class needs to be called every loop
 * to work properly.
 */
public class LatchedBoolean {
  ...
  /**
   * Creates a latched boolean with the mode "Always" in which between False->True and True->False
   ...
   * @param startValue the value to start at (if opposite of first value, latched boolean will be
   *     triggered)
   */
  ...
  public boolean get(boolean currentValue) {
    ...
    if (changed) {
      if (mode == LatchedBooleanMode.ALWAYS) return true;
      if (mode == LatchedBooleanMode.RISING && currentValue == true) return true;
