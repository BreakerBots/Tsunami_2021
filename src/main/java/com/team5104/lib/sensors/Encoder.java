...
public abstract class Encoder {
  ...
  // Sub Classes
  private abstract static class CTRE_Encoder extends Encoder {
  ...
  public static class EncoderSim extends MagEncoder {
    ...
    public void setVelocityAccelMeters(
        double metersPerSecond, double metersPerSecondSquared, double wheelDiameter) {
      setTicks(
          componentRevsToTicks(Units.metersToFeet(metersPerSecond) / (wheelDiameter * Math.PI)));
      setTicksVel(
          componentRevsToTicks(
                  Units.metersToFeet(metersPerSecondSquared) / (wheelDiameter * Math.PI))
              / 10d);
    }
  }
}
