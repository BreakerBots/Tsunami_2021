...
public class Limelight {
  ...
  public static NetworkTableEntry getEntry(String key) {
    if (table != null) return table.getEntry(key);
    else return null;
  }

  public static void setEntry(String key, double entry) {
    if (table != null) getEntry(key).setDouble(entry);
  }

  public static double getDouble(String key, double defaultValue) {
    if (table != null) return getEntry(key).getDouble(defaultValue);
  ...
  public static double getLatency() {
    return getDouble("tl", 0);
  }

  public static boolean hasTarget() {
    return getDouble("tv", 0) == 1;
  }

  public static boolean isConnected() {
    return getDouble("tl", 0) != 0.0;
  }
  ...
  public static void setLEDMode(LEDMode ledMode) {
    if (isConnected()) setEntry("ledMode", ledMode.value);
        ...
        new Notifier(
            () -> {
              if (connected.get(isConnected())) {
                if (defaultOff) setLEDMode(LEDMode.OFF);
