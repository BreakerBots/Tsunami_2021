...
/**
 * A wrapper class for the REV Color Sensor V3 that rounds the color to the nearest of the constant
 * colors.
 */
public class ColorSensor {
  ...
  public PanelColor getNearestColor() {
    ...
    for (int i = 0; i < colors.length; i++) {
      if (colors[i].distance(color) < colors[closestIndex].distance(color)) closestIndex = i;
    }
  ...
  public PercentColor getRawColor() {
    return new PercentColor(
        colorSensor.getColor().red, colorSensor.getColor().green, colorSensor.getColor().blue);
  }
