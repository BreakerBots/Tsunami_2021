...
public class console {
  ...
  /**
   * Prints out text in a separate thread (lowest priority) and adds prints to a text file (if
   * Console.logFile.isLogging) Examples: 2.12 [Robot]: Message 90.12 ERROR [subsystems.Turret]:
   * Message
   */
  private static void logBase(int stackCount, Type type, Object... data) {
    ...
    String f =
        round(Timer.getFPGATimestamp(), 2)
            + ": "
            + type.message
            + "["
            + location
            + "]: "
            + parseAndRound(2, data);
    addToPrintBuffer(f);
    if (logFile.isLogging) logFile.log += f + "\n";
  }
  /** Prints out text to the console under the type "INFO" and category "OTHER" */
  public static void log(Object... data) {
    logBase(2, Type.INFO, data);
  }
  /** Prints out text to the console under the type "ERROR" and category "OTHER" */
  public static void error(Object... data) {
    logBase(2, Type.ERROR, data);
  }
  /** Prints out text to the console under the type "WARN" and category "OTHER" */
  public static void warn(Object... data) {
    logBase(2, Type.WARNING, data);
  }

  // -- Parse

  /**
   * Parses multiple objects into one string (no rounding). The same function as used when normally
   * calling console.log()
   */
  ...
  /**
   * Rounds all doubles, then parses multiple objects into one string. The same function as used
   * when normally calling console.log().
   */
  ...
  /**
   * Parts a list of objects into a CC (colon-comma) pattern. Output will be "v1: v2, v3: v4..."
   * Rounds all doubles to 2 decimal places
   */
  ...
  public static class sets {
    ...
    public static void create(String name) {
      if (getIndex(name) != -1) {
        ...
        si++;
      } else console.error("Max Amount of Sets Created");
    }
    ...
    /**
     * Returns the time (in seconds) of the corresponding timing group/set. Returns -1 if nothing
     * found
     */
    ...
    /**
     * Returns the time (in specified format) of the corresponding timing group/set. Returns -1 if
     * nothing found
     */
    ...
    public static void log(String timingGroupName, Object... a) {
      console.logBase(
          2, Type.INFO, parse(a) + " " + String.format("%.2f", getTime(timingGroupName)) + "s");
    }
  }
  ...
  public static class logFile {
    ...
    /**
     * Starts Recording the "console.logs" into a string that can be saved with
     * "console.logFile.end()" If the logger is already logging it will not clear the log, but just
     * do nothing
     */
    ...
    /**
     * Saves the log string that is logged onto after calling "console.logFile.start()" Match =>
     * "MatchLog" if Constants.SaveMatchLogs Other => "GeneralLog" if Constants.SaveNonMatchLogs
     */
    public static void end() {
      try {
        if (isLogging) {
          ...
          // File Name
          if (hasFMS ? OVERWRITE_MATCH_LOGS : OVERWRITE_NON_MATCH_LOGS) fileName = "log.txt";
          else
            fileName =
                DateTimeFormatter.ofPattern("MM-dd-yyyy_HH-mm").format(LocalDateTime.now())
                    + ".txt";
  ...
  public static class timedPrinter {
    ...
    private static TimedPrinter get(String printerId, int deltaLoops) {
      ...
      for (TimedPrinter printer : printers) {
        if (printer.id.equals(printerId)) return printer;
      }
