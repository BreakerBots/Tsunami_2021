...
public class Hopper extends Subsystem {
  ...
  public void update() {
    ...
    // Indexing
      ...
      // Entry
      if (Superstructure.is(Mode.INTAKE)) setStart(Constants.hopper.START_SPEED_INTAKING);
      else if (isIndexing) setStart(Constants.hopper.START_SPEED_INDEXING);
      else setStart(0);
    }
  ...
  // Debugging
  public void debug() {
    // Constants.HOPPER_INDEX_BALL_SIZE = Tuner.getTunerInputDouble("Hopper Indexer Ball Size",
    // Constants.HOPPER_INDEX_BALL_SIZE);
    ...
    Constants.hopper.TOLERANCE =
        Tuner.getTunerInputDouble("Hopper Index Tol", Constants.hopper.TOLERANCE);
    Constants.hopper.FEED_SPEED =
        Tuner.getTunerInputDouble("Hopper Feed Speed", Constants.hopper.FEED_SPEED);
  ...
  public void init() {
    ...
    entrySensor =
        new Sensor(
            PortType.ANALOG, Ports.HOPPER_SENSOR_START, Constants.robot.switchOnBot(false, true));
    endSensor =
        new Sensor(
            PortType.ANALOG, Ports.HOPPER_SENSOR_END, Constants.robot.switchOnBot(false, true));
    ...
    controller = new PIDController(Constants.hopper.KP, Constants.hopper.KI, Constants.hopper.KD);
