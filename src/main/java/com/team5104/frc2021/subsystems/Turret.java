...
public class Turret extends Subsystem {
  ...
  public void update() {
    ...
    if (Superstructure.isEnabled()) {
      ...
      // Vision
        ...
        if (
        /*Limelight.hasTarget() &&*/ Superstructure.is(Mode.AIMING)) {
          setAngle(
              latencyCompensator.getValueInHistory(Limelight.getLatency())
      ...
      // Field Oriented Mode
        ...
        setAngle(
            Util.wrap180(Drive.getHeading() + fieldOrientedOffset + tunerFieldOrientedOffsetAdd));
    }
  ...
  public static boolean leftLimitHit() {
    ...
    else if (Constants.robot.id == 0) return motor.isFwdLimitSwitchClosed() == 1;
  ...
  public void init() {
    ...
    motor.configForwardSoftLimitThreshold(
        (int) encoder.componentRevsToTicks(Constants.turret.SOFT_LEFT / 360d));
    motor.configReverseSoftLimitThreshold(
        (int) encoder.componentRevsToTicks(Constants.turret.SOFT_RIGHT / 360d));
    enableSoftLimits(false);
