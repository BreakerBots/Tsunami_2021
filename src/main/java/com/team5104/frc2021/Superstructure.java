...
public class Superstructure {
  ...
  public static void set(PanelState panelState) {
    Superstructure.panelState = panelState;
  }

  public static boolean is(PanelState panelState) {
    return panelState == Superstructure.panelState;
  }

  public static void set(FlywheelState shooterWheelState) {
    Superstructure.flywheelState = shooterWheelState;
  }

  public static boolean is(FlywheelState flywheelState) {
    return flywheelState == Superstructure.flywheelState;
  }

  public static void set(Target target) {
    Superstructure.target = target;
  }

  public static boolean is(Target target) {
    return target == Superstructure.target;
  }
  ...
  protected static void update() {
    ...
    if (flywheelOnTarget.get(Flywheel.isSpedUp()) && is(Mode.AIMING)) console.log("sped up");
    if (hoodOnTarget.get(Hood.onTarget()) && is(Mode.AIMING)) console.log("on target");
    if (turretOnTarget.get(Turret.onTarget()) && is(Mode.AIMING)) console.log("on target");
    readyToFire.update(
        is(Mode.AIMING)
            && Flywheel.isSpedUp()
            && Turret.onTarget()
    ...
    if (limelightOn.get(is(Mode.AIMING) || is(Mode.SHOOTING))) {
      if (is(Mode.AIMING) || is(Mode.SHOOTING)) Limelight.setLEDMode(LEDMode.ON);
      else if (Limelight.defaultOff) Limelight.setLEDMode(LEDMode.OFF);
    }
  }
